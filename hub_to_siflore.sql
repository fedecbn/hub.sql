...
EXECUTE 'TRUNCATE exploitation.taxons; 
INSERT INTO exploitation.taxons 
SELECT cd_ref, nom_complet, rang, CASE WHEN liste_bryo IS TRUE AND bryophyta IS TRUE THEN ''bryo_liste'' WHEN liste_bryo IS FALSE AND bryophyta IS TRUE THEN ''bryo_pas_liste'' WHEN liste_bryo IS FALSE AND bryophyta IS FALSE THEN ''tracheo'' ELSE ''problème'' END as type
FROM exploitation.taxref_v'||version||'_new;';
...
cmd = 'SELECT typ_geo, cd_geo, cd_jdd FROM hub.metadonnees_territoire LIMIT 1 OFFSET 8;';
