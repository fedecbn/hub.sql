...
------------------------------------------
CREATE OR REPLACE FUNCTION hub_ref(typAction varchar, path varchar = null,  delimitr varchar = ';') RETURNS setof zz_log AS 
$BODY$ DECLARE out zz_log%rowtype; DECLARE flag1 integer; DECLARE flag2 integer; DECLARE champFonction varchar; DECLARE libTable varchar; DECLARE structure varchar; BEGIN
--- Output
out."libSchema" := '-';out."libTable" := '-';out."libChamp" := '-';out."typLog" := 'hub_ref';out."nbOccurence" := 1; SELECT CURRENT_TIMESTAMP INTO out."date";
---Variables
DROP TABLE IF  EXISTS public.ref_meta;CREATE TABLE public.ref_meta (id varchar, structure varchar, CONSTRAINT ref_meta_pk PRIMARY KEY(id));
...
WHEN typAction = 'create' THEN	--- Creation
	...
	FOR libTable IN EXECUTE 'SELECT id FROM public.ref_meta'
		LOOP EXECUTE 'SELECT DISTINCT 1 FROM pg_tables WHERE schemaname = ''ref'' AND tablename = '''||libTable||''';' INTO flag2;
		CASE WHEN flag2 = 1 THEN 
			out."libLog" := 'Tables ref.ddd et ref.fsd ont déjà été créés';RETURN next out;
		...
		EXECUTE 'COPY ref.'||libTable||' FROM '''||path||'std_'||libTable||'.csv'' HEADER CSV DELIMITER '''||delimitr||''' ENCODING ''UTF8'';'; out."libLog" := libTable||' : données importées';RETURN next out;
		END CASE;
		END LOOP;
WHEN typAction = 'update' THEN	--- Mise à jour
	FOREACH libTable IN ARRAY listTable
		...
		CASE WHEN flag2 = 1 THEN
			...
			EXECUTE 'COPY ref.'||libTable||' FROM '''||path||'std_'||libTable||'.csv'' HEADER CSV DELIMITER '''||delimitr||''' ENCODING ''UTF8'';';
